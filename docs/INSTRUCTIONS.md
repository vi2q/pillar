...
3. **pillar-coding-agent**: 着手。小規模 core 群 + auth-storage/models-store + model-runtime/runtime-credentials + messages + compaction (utils/driver) + session-entries/branch-summarization 完了。次の候補: session-manager (1.7k行) / settings-manager (1.4k行) / package-manager (2.7k行) / agent-session (3.5k行) / export-html (746行)。
