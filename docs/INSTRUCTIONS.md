...
| coding-agent | 49.4k | 🔶 **約31%** (core 29.5k のうち約52%移植済み, rs 16.5k / 468テスト)。残り: agent-session 4.2k / package-manager 2.7k / extensions ランナー 4.1k / resource-loader 1.1k / sdk 0.4k / core/tools bash+powershell 0.6k / cli 1.9k / utils 3.6k / modes 20.3k |
...
3. **pillar-coding-agent**: 着手。小規模 core 群 + auth-storage/models-store + model-runtime/runtime-credentials + messages + compaction + session-entries/branch-summarization + session-manager + settings-manager + trust-manager/pi-manifest + export-html + usage-totals 群 + truncate/strip-ansi/sanitize + bash-executor/exec + tools (output-accumulator/path-utils/render-utils/file-mutation-queue/ls/search/read/write) 完了。find/grep は FFF 設計参考のネイティブ実装に置換済み。残り: tools/edit (461行) / package-manager (2.7k行) / agent-session (3.5k行) / extensions ランナー系。
