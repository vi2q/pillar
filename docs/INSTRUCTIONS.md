...
| coding-agent | 78.9k | 50.3k | 🔶 着手 (system-prompt 16 + prompt-templates 19 + keybindings 9 + model-config 12 + skills 22 + resolve-config-value 24 + cache-stats 16 + model-resolver 48 + provider-composer 32 + 小規模 core 群 20 + auth-storage/models-store 11 + model-runtime 15 + messages 18 + compaction 23 + session-entries/branch-summarization 11 + session-manager 22 + settings-manager 22 + trust-manager/pi-manifest 16 + export-html 10 + truncate 17 + bash-executor/exec 10 テスト) |
...
3. **pillar-coding-agent**: 着手。小規模 core 群 + auth-storage/models-store + model-runtime/runtime-credentials + messages + compaction + session-entries/branch-summarization + session-manager + settings-manager + trust-manager/pi-manifest + export-html + usage-totals 群 + truncate/strip-ansi/sanitize + bash-executor/exec 完了。残り: package-manager (2.7k行) / agent-session (3.5k行) / extensions ランナー系 / tools (bash/read/edit/write/grep/find/ls 4.8k行)。
