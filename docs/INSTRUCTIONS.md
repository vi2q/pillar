...
| coding-agent | 78.9k | 50.3k | 🔶 着手 (system-prompt 16 + prompt-templates 19 + keybindings 9 + model-config 12 テスト) |
...
3. **pillar-coding-agent**: 着手。system-prompt (16) + prompt-templates (19) + keybindings (9) + model-config (12) 完了。次の候補: skills のディレクトリ走査 (ignore パターン), model-resolver (782行) / provider-composer (572行) → その後 agent-session (3.5k行, 依存多数) と package-manager (2.7k行)。
