...
| coding-agent | 78.9k | 50.3k | 🔶 着手 (system-prompt 16 + prompt-templates 19 + keybindings 9 + model-config 12 + skills 22 + resolve-config-value 24 + cache-stats 16 + model-resolver 48 + provider-composer 32 + 小規模 core 群 20 テスト) |
...
3. **pillar-coding-agent**: 着手。小規模 core 群 (provider-attribution/timings/source-info/auth-guidance/diagnostics/slash-commands/event-bus/session-cwd/cache-stats/model-resolver/provider-composer) 完了。次の候補: models-store (147行) → model-runtime (787行) → agent-session (3.5k行) / package-manager (2.7k行) / settings-manager (1.4k行) / session-manager (1.7k行)。
