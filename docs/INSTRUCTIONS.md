...
- [ ] luaur follow-up PR #40 のレビュー (2026-09-02)
  - [x] PR 本文・実装・PR #37 コメントを確認し、異議と修正文案を提示する (2026-09-02)
    - 要修正: tuple の `FromLuaMulti` が `from_stack_multi` を override しておらず、`eval::<(f64,)>()` 等では引き続き `-0.0` の符号を失う (mlua の tuple 実装は stack fast path を伝播する)。再現テストで確認
    - テスト記述: Rust→Lua→Rust の素通しケースが実装されておらず、同一 assertion が重複。PR 本文の説明とも不一致
    - 添削: f32 の value-level conversion error は従来の `to: f64` から `to: f32` に変わるため「error messages unchanged」は不正確。serde 回帰ケースはコミット済みテスト化を推奨
    - PR #37 コメント自体は適切。#40 が後継であるため #37 は close 推奨
  - [ ] Confirm (user): 添削・レビュー結果の承認 — pending confirmation
