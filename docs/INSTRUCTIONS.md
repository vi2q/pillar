...
2. **pillar-ai のプロバイダ残り**: カタログジェネレータ (39 providers / 1371 モデル生成済み) と builtin provider 登録は完了。残りは stream dispatch の per-API option 変換の結線 (`StreamRequestOptions` → 各アダプタの Options 構造体) のみ。`cargo run -p pillar-ai --bin generate-models` で `models_generated.rs` を再生成できる。
