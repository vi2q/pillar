...
| coding-agent | 78.9k | 50.3k | 🔶 着手 (system-prompt 16 + prompt-templates 19 + keybindings 9 + model-config 12 + skills 22 + resolve-config-value 24 + cache-stats 16 + model-resolver 48 テスト) |
...
3. **pillar-coding-agent**: 着手。小規模 core 群 + cache-stats + model-resolver 完了。次の候補: provider-composer (572行) → agent-session (3.5k行, 依存多数) と package-manager (2.7k行)。
