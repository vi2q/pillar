...
- `pillar-agent`'s OS-bound surface is behind default-on features: `harness-tools` (the coding agent's file/shell tools plus the `std`/`tokio` execution environment), `session-files` (the durable JSONL file backend), `search` (the native session-search scanner). `cargo check -p pillar-agent --no-default-features` is the LMPC-minimum shape and compiles natively and for `wasm32-unknown-unknown`; `dependency_profiles::the_embedding_core_keeps_os_capabilities_behind_features` fails if any *other* core source reaches `std::process` / `std::fs` / `std::net`.
