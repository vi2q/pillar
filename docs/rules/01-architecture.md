...
- `pillar-lmpc::host_model` stores and resumes conversations (`messages_json` / `restore`, the `lmpc_session_export` / `lmpc_session_import` ABI calls): the host owns persistence — it keeps the JSON and hands it back to a fresh session. The ABI keeps the two ends symmetric: export refuses a state past `lmpc_session_state_cap` (4 MiB) with `-1`, a `length` past the buffer it is read into is refused rather than clamped, and a state larger than the 64 KiB input buffer goes back in through `lmpc_session_import_begin` / `…_write` / `…_commit` (sequential byte chunks, validated at the commit).
