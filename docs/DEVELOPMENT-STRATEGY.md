...
2. 同じ核をnative hostとazparam用Wasm hostで駆動できる。compiledだけでなくstreaming・cancel・保存／復帰・shutdownまで実行する。
   進捗: **前提のみ達成**。LMPC最小（`pillar-ai`＋`pillar-agent`）とLMPC＋Luau（＋`pillar-extensions`）は`wasm32-unknown-unknown`でコンパイルできる（`scripts/check.sh`のゲート）。実行は未検証で、阻害要因はTASKSに列挙（`tokio::spawn`のホスト駆動化、harnessのOS境界分離、拡張の読み込みをhost供給へ、`tokio::time`依存）。
