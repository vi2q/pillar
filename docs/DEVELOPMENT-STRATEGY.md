...
- **capability台帳**: 能力ID、対象profile、提供API、所有adapter、必要権限、状態（未実装／配線済み／検証済み）、型チェック例、成功・失敗・中断のテスト、性能corpusへの参照。初版は `capabilities.json`（`api` = 公開済み、`gap` = 未公開の必要能力、`scenario` = 制作シナリオ）。ゲートは `crates/pillar-extensions/tests/capability_ledger.rs`: `surface = "luau"` の行は VM で歩いた実際の公開面と双方向で一致すること、`verified` は実在するテストを要求すること（**未配線の公開を防ぐ**）。
